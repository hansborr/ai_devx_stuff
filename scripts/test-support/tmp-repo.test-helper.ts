...
// Shared temp-repo scaffold for the scripts test suites. Every scripts test
// that needs a working directory previously hand-rolled the same lifecycle: a
// module-level string[] of roots, a drain hook, and a writeRepo/makeRepo builder
// calling mkdtempSync + mkdirSync/writeFileSync. This helper centralizes the
// mkdtempSync build and the rmSync(..., { recursive: true, force: true })
// cleanup so a future leak fix or git-init-flag change is a single edit, and
// standardizes the defensive `root !== undefined` drain guard.
//
...
const DEFAULT_PREFIX = "musi-scripts-tmp-repo-";
