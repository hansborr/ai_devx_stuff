...
- The hook wiring in `.claude/settings.json` and the whole `.codex/hooks.json`
  ...
  `bun run harness:wiring:check`). `.codex/hooks.json` must keep Codex's
  supported top-level shape, so generated-file ownership is enforced by the
  freshness check instead of an in-file marker. The `hooks` key in
  `.claude/settings.json` is generated too.
