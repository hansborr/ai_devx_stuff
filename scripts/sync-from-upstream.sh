...
#   AGENTS.md, CLAUDE.md this clone's own agent orientation ("this is a partial
#                        mirror; here's what works"). Upstream's brief is instead
#                        re-copied to docs/upstream-AGENTS.md below.
#   docs/upstream-AGENTS.md  generated by this script (upstream AGENTS.md plus a
#                        provenance header); excluded from the mirror passes so
#                        the delete pass never orphans it.
#   bun.lock             locks this clone's actual manifests (root + stub
#                        packages/**), so installs leave a clean tree and frozen
#                        installs work. Upstream's lockfile describes ~963 lines
#                        of workspace deps for app code that isn't here. After a
#                        sync changes root package.json, run `bun install` to
#                        refresh it.
...
PRESERVE_REGEX='^(packages/|README\.md$|AGENTS\.md$|CLAUDE\.md$|docs/upstream-AGENTS\.md$|bun\.lock$)|sync-from-upstream|^\.claude/statusline\.sh$'
...
echo "    preserved (untouched): packages/**, README.md, AGENTS.md, CLAUDE.md, bun.lock, the sync-from-upstream skill, .claude/statusline.sh"
echo "    regenerated on apply:  docs/upstream-AGENTS.md (from upstream AGENTS.md)"
echo
...
# Re-copy upstream's agent brief (with a provenance header) — the local
# AGENTS.md is this clone's own orientation doc, so the real brief is kept
# here as an example artifact instead of being mirrored over it.
if [ -f "$UPSTREAM/AGENTS.md" ]; then
  {
    printf '> **Preserved upstream artifact.** This is the always-loaded agent brief\n'
    printf '> (`AGENTS.md`) from the upstream Musi repo, kept here as an example of the\n'
    printf '> feedforward-context pattern the harness docs describe. Its commands and\n'
    printf '> workflow rules apply to the full upstream repo, **not** to this partial\n'
    printf '> clone — see this repo'"'"'s own [`AGENTS.md`](../AGENTS.md). Re-copied on\n'
    printf '> every `sync-from-upstream` run; do not edit by hand.\n\n'
    cat "$UPSTREAM/AGENTS.md"
  } > docs/upstream-AGENTS.md
  echo "  regenerated: docs/upstream-AGENTS.md"
fi
