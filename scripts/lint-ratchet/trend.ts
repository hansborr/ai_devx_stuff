...
function baselineHistory(options: RunLintRatchetTrendOptions): readonly BaselineCommit[] {
  return parseBaselineLog(gitOutput(logArgs(options), options.deps ?? defaultTrendDeps));
}
...
function historicalBaselineText(
  ...
  try {
    return gitOutput(["show", `${commit.sha}:${commit.path}`], deps);
  } catch {
    return undefined;
  }
}
...
export function formatLintRatchetTrend(
  ...
  activeRatchetIds: ReadonlySet<string> = new Set(lintRatchets.map((ratchet) => ratchet.id)),
): string {
...
export function runLintRatchetTrend(
  ...
  const activeRatchetIds = new Set((options.ratchets ?? lintRatchets).map((ratchet) => ratchet.id));
...
export function runLintRatchetTrendCli(
  since: string | undefined,
  max: number | undefined,
  includeRetired: boolean | undefined,
): void {
  const result = runLintRatchetTrend({ since, max, includeRetired });
