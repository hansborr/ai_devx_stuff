...
type ParsedCliOptions = {
  scopeMode: ScopeMode; base: string; baseExplicit: boolean; format: CliOptions["format"];
  roots: string[];
  configPath?: string; outputPath?: string; chunkDir?: string; chunkSize?: number;
  requested: DriftCheckId[]; allRequested: boolean;
};

function initialParsedOptions(): ParsedCliOptions {
  return { scopeMode: DEFAULT_SCOPE_MODE, base: DEFAULT_BASE, baseExplicit: false, format: "text", roots: [], requested: [], allRequested: false };
}
...
function parseScopeOption(arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions): number | undefined {
  const option = readOptionValue(arg, argv, index);
  if (option.value !== "changed" && option.value !== "current") throw new DriftAiError("--scope requires changed or current.");
  parsed.scopeMode = option.value;
  return option.nextIndex;
}

function parsePathOption(arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions): number | undefined {
  const optionName = optionNameFor(arg);
  const option = readOptionValue(arg, argv, index);
  if (!option.value) throw new DriftAiError(optionName === "--base" ? "--base requires a ref." : `${optionName} requires a path.`);
  if (optionName === "--base") {
...
function parseOutputOption(arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions): number | undefined {
  if (optionNameFor(arg) === "--format") {
    const option = readOptionValue(arg, argv, index);
    if (option.value !== "text" && option.value !== "json") throw new DriftAiError("--format requires text or json.");
...
function parseChunkOption(arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions): number | undefined {
  ...
  const option = readOptionValue(arg, argv, index);
  if (!/^[1-9]\d*$/u.test(option.value)) throw new DriftAiError("--chunk-size requires a positive integer.");
  parsed.chunkSize = Number(option.value);
  return option.nextIndex;
}

function parseCheckOption(arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions): number | undefined {
  const option = readOptionValue(arg, argv, index);
  if (!option.value) throw new DriftAiError("--check requires duplicates, ghost-files, comments, suppressions, or all.");
  if (option.value === "all") {
    parsed.allRequested = true;
    return option.nextIndex;
  }
  if (!isCheckId(option.value)) throw new DriftAiError(`Unknown check: ${option.value}\n${usage()}`);
  if (!parsed.requested.includes(option.value)) parsed.requested.push(option.value);
  return option.nextIndex;
}

type OptionParser = (arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions) => number | undefined;
...
function parseKnownOption(arg: string, argv: readonly string[], index: number, parsed: ParsedCliOptions): number | undefined {
  return OPTION_PARSERS[optionNameFor(arg)]?.(arg, argv, index, parsed);
}

function validateScopeOptions(parsed: ParsedCliOptions): void {
  if (parsed.scopeMode === "current" && parsed.baseExplicit) throw new DriftAiError("--scope current does not accept --base; current scope has no merge base.");
  if (parsed.scopeMode === "changed" && parsed.roots.length > 0) throw new DriftAiError("--root is only valid with --scope current.");
}

function validateChunkOptions(parsed: ParsedCliOptions): void {
  if (parsed.chunkSize === undefined) return;
  if (parsed.chunkDir === undefined) throw new DriftAiError("--chunk-size is only valid with --chunk-dir.");
}
...
function cliOptionsFromParsed(parsed: ParsedCliOptions): CliOptions {
  return {
    ...
    checks: parsed.allRequested || parsed.requested.length === 0 ? [...ALL_CHECKS] : parsed.requested,
