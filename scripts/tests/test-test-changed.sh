...
mkdir -p "$repo/scripts/logs-audit"
printf 'changed\n' > "$repo/scripts/logs-audit/logs-audit-checks.ts"
git -C "$repo" add scripts/logs-audit/logs-audit-checks.ts
: > "$repo/bun.log"
...
mkdir -p "$repo/scripts/logs-audit"
printf 'changed\n' > "$repo/scripts/logs-audit/logs-audit.test.ts"
git -C "$repo" add scripts/logs-audit/logs-audit.test.ts
: > "$repo/bun.log"
...
repo="$(new_repo package-scripts-change)"
printf '{"scripts":{"test":"vitest","lint":"eslint ."},"devDependencies":{"vitest":"1.0.0"}}\n' > "$repo/package.json"
: > "$repo/bun.log"
run_test_changed "$repo" >/dev/null || fail "package script change should run"
grep -qF 'stub vitest run --passWithNoTests --project=scripts' "$repo/bun.log" \
  || fail "package script-only change should run scripts project: $(cat "$repo/bun.log")"
if grep -q -- '--changed' "$repo/bun.log"; then
  fail "package script-only changes should run scripts project in full: $(cat "$repo/bun.log")"
fi
ok "package script-only changes run scripts project tests"

repo="$(new_repo package-dependency-change)"
...
: > "$repo/bun.log"
run_test_changed "$repo" >/dev/null || fail "scripts tsconfig change should run"
grep -qF 'stub vitest run --passWithNoTests --project=scripts' "$repo/bun.log" \
  || fail "scripts tsconfig change should run scripts project: $(cat "$repo/bun.log")"
if grep -q -- '--changed' "$repo/bun.log"; then
  fail "scripts tsconfig changes should run scripts project in full: $(cat "$repo/bun.log")"
fi
...
ok "eslint config changes run config-dependent project tests"

repo="$(new_repo root-eslint-config-change)"
printf 'export default [{ rules: {} }];\n' > "$repo/eslint.config.js"
git -C "$repo" add eslint.config.js
: > "$repo/bun.log"
output="$(run_test_changed "$repo")" || fail "root eslint config change should run: $output"
if grep -qF 'test:changed: no Vitest-relevant changes' <<< "$output"; then
...
ok "root eslint config changes run config-dependent project tests"
...
# the missing-ref case.
repo="$(new_repo orphan-branch)"
git -C "$repo" checkout -q --orphan orphan
git -C "$repo" commit -qm orphan-seed
: > "$repo/bun.log"
output="$(run_test_changed "$repo" 2>&1)" || fail "disjoint base should fall back to full suite: $output"
grep -qF "'main' shares no history with HEAD" <<< "$output" \
  || fail "disjoint base fallback should be announced: $output"
grep -qF 'stub vitest run --passWithNoTests' "$repo/bun.log" \
  || fail "disjoint base should run full Vitest suite: $(cat "$repo/bun.log")"
ok "base with no common ancestor falls back to full Vitest suite"
