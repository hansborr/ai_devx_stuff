...
# smoke-subjects: scripts/lint-ratchet/output-diagnostics-file.test.ts
# smoke-subjects: scripts/lint-ratchet/output-drift.test.ts
# smoke-subjects: scripts/lint-ratchet/output-emission.test.ts
# smoke-subjects: scripts/lint-ratchet/output-fixture.test-helper.ts
# smoke-subjects: scripts/lint-ratchet/output-propose.test.ts
# smoke-subjects: scripts/lint-ratchet/output-update-collection.test.ts
# smoke-subjects: scripts/lint-ratchet/output-warnings.test.ts
...
# smoke-subjects: examples/lint-ratchet-demo/scripts/lint-ratchet.ts
# smoke-subjects: examples/lint-ratchet-demo/scripts/lint-ratchet/adapter.ts
...
# in leaf 02 S5; assert_portable_runtime_closure keeps the remaining list closed
# over static repository-local imports without restoring that machinery.)
...
assert_portable_runtime_closure() {
  local runtime_files
  runtime_files=$(printf '%s\n' "${PORTABLE_RUNTIME_FILES[@]}")
  if ! MUSI_LINT_RATCHET_CLOSURE_ROOT="$REPO_ROOT" \
    MUSI_LINT_RATCHET_RUNTIME_FILES="$runtime_files" bun -e '
      const root = process.env.MUSI_LINT_RATCHET_CLOSURE_ROOT;
      const runtimeFiles = process.env.MUSI_LINT_RATCHET_RUNTIME_FILES?.split("\n") ?? [];
      const entry = "scripts/lint-ratchet.ts";
      const config = "scripts/lint-ratchet/lint-ratchet-config.ts";
      if (!runtimeFiles.includes(entry)) {
        console.error(`portable runtime copy set is missing adapter entry: ${entry}`);
        process.exit(1);
      }
      const { validateSeedImportClosure } = await import(
        `${root}/scripts/import-closure/closure-walk.ts`
      );
      const { violations } = validateSeedImportClosure({
        root,
        entry,
        allowedRoots: [],
        allowedFiles: [...runtimeFiles, config],
        terminalFiles: [config],
        externalPackages: ["@musi/harness-diagnostics", "@musi/lint-ratchet"],
        nonStaticSpecifiers: "skip",
      });
      if (violations.length > 0) {
        console.error(
          `portable runtime copy set is missing repository-local adapter import(s):\n${violations
    ...
    '
  then
    fail "portable runtime copy closure is incomplete"
  fi
}

assert_portable_runtime_closure

# The demo is a workspace nested inside this monorepo, so its adapter root does
# not match `git rev-parse --show-toplevel`. The advisory checker deliberately
# degrades to a warning on that load failure; pin the copyable package-script
# repair command because the package bin is not linked in this workspace layout.
set +e
(
  cd examples/lint-ratchet-demo
  env -u CI -u MUSI_SUPPRESS_MERGE_DRIVER_WARN \
    bun scripts/lint-ratchet.ts --check-baseline \
    >"$TMP_ROOT/demo-presence-check.out" 2>"$TMP_ROOT/demo-presence-check.err"
)
set -e
grep -qF \
  'WARN: lint-ratchet merge driver is missing or stale - run bun run lint:ratchet:install-merge-driver' \
  "$TMP_ROOT/demo-presence-check.err" \
  || fail "demo presence checker did not preserve its workspace-safe repair command: $(cat "$TMP_ROOT/demo-presence-check.err")"
...
write_fixture_install_marker() {
  local fixture_dir=$1
  (
    cd "$fixture_dir"
    # shellcheck source=scripts/dependency-freshness.sh
    . scripts/dependency-freshness.sh
    musi_dependency_write_marker "$fixture_dir"
  )
}
...
build_fixture() {
  ...
  for runtime_file in "${PORTABLE_RUNTIME_FILES[@]}"; do
    ...
    # statically. The generic copy-set guard therefore still skips this loop;
    # assert_portable_runtime_closure checks this exact dynamic list before any
    # sandbox is built.
  ...
  # fixture needs zod at its own node_modules root.
  ln -s "$REPO_ROOT/node_modules/zod" "$fixture_dir/node_modules/zod"
  # The copied adapter imports @musi/lint-ratchet/* and @musi/harness-diagnostics/*;
  # resolve both to their source-only packages via scoped node_modules symlinks
  # (leaf 02 S3, leaf 181).
  mkdir -p "$fixture_dir/node_modules/@musi"
  ln -s "$REPO_ROOT/tools/lint-ratchet" "$fixture_dir/node_modules/@musi/lint-ratchet"
  ln -s "$REPO_ROOT/tools/harness-diagnostics" \
    "$fixture_dir/node_modules/@musi/harness-diagnostics"
...
JSON
  ...
  cp "$REPO_ROOT/bun.lock" "$fixture_dir/bun.lock"
  # The smoke models a completed Musi install. Individual package symlinks
  # stand in for node_modules, while the real helper writes the same digest
  # marker that postinstall would create.
  write_fixture_install_marker "$fixture_dir"
...
assert_local_identity_regression() {
  ...
  identity_paths=$(bun -e '
      ...
      for (const id of cases) {
        ...
        paths.push(eslintCachePathFor(ratchet, hash, binding));
      }
...
JS
  ...
  # Replacing node_modules above also replaces the install marker.
  write_fixture_install_marker "$fixture_dir"
}
...
MISSING_LOCK_DIR="$TMP_ROOT/missing-lock"
build_fixture "$MISSING_LOCK_DIR"
write_type_assertion_config "$MISSING_LOCK_DIR"
write_clean_source "$MISSING_LOCK_DIR"
ensure_fixture_git_index "$MISSING_LOCK_DIR"
rm "$MISSING_LOCK_DIR/bun.lock"
set +e
(cd "$MISSING_LOCK_DIR" && bun run scripts/lint-ratchet.ts --update \
  >"$TMP_ROOT/missing-lock.out" 2>"$TMP_ROOT/missing-lock.err")
status=$?
set -e
[ "$status" -eq 2 ] \
  || fail "missing-lock update should exit 2, got $status: $(cat "$TMP_ROOT/missing-lock.err")"
grep -qF "cannot verify install freshness without bun.lock" "$TMP_ROOT/missing-lock.err" \
  || fail "missing-lock update should name bun.lock recovery: $(cat "$TMP_ROOT/missing-lock.err")"

MTIME_FALLBACK_DIR="$TMP_ROOT/mtime-fallback"
build_fixture "$MTIME_FALLBACK_DIR"
write_type_assertion_config "$MTIME_FALLBACK_DIR"
write_clean_source "$MTIME_FALLBACK_DIR"
rm "$MTIME_FALLBACK_DIR/node_modules/.musi-install-digest"
touch -t 200001010000 "$MTIME_FALLBACK_DIR/bun.lock"
run_fixture_update "$MTIME_FALLBACK_DIR" \
  || fail "missing-marker mtime fallback should allow update: $(cat "$TMP_ROOT/update.err")"
...
set -e
  [ "$status" -eq 3 ] \
    || fail "third-party check-baseline should exit 3, got $status: $(cat "$TMP_ROOT/third-baseline-check.err")"
