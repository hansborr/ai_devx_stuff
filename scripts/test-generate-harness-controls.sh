#!/usr/bin/env bash
# Smoke test for scripts/generate-harness-controls.ts.
#
# Contract:
...
run_fixture_render_check() {
  ...
  (cd "$fixture_dir" && bun run scripts/generate-harness-controls.ts >"$TMP_ROOT/render.out" 2>"$TMP_ROOT/render.err")
...
run_failure_case() {
  ...
  if (cd "$fixture_dir" && bun run scripts/generate-harness-controls.ts >"$TMP_ROOT/failure-$name.out" 2>"$stderr_path"); then
...
JSON
  ...
  if (cd "$fixture_dir" && bun run scripts/generate-harness-controls.ts >"$TMP_ROOT/failure-duplicate-id.out" 2>"$stderr_path"); then
...
run_unknown_arg_case() {
  ...
  if (cd "$fixture_dir" && bun run scripts/generate-harness-controls.ts --bogus >"$TMP_ROOT/failure-unknown-arg.out" 2>"$stderr_path"); then
...
run_manifest_shape_case() {
  ...
  if (cd "$fixture_dir" && bun run scripts/generate-harness-controls.ts >"$TMP_ROOT/failure-manifest-shape-$name.out" 2>"$stderr_path"); then
