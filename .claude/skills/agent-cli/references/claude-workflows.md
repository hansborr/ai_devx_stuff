...
- Parallel implementation wrappers need `isolation: 'worktree'`: edits collide
  ...
  worktree anyway. Consults are lock-free on every backend, so parallel
  consult shims need no isolation. Musi repo-local: fresh worktrees may need
  provisioning (`docs/guides/per-worktree-dev.md`); build `@musi/shared`
  before `worktree:init` when seed inputs changed on the base.
