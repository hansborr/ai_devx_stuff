...
Mission clauses that each cost a retry round when missing:

- **Ask for a self-contained final message** ("output only the findings; do not refer to analysis above"). `-o` captures only the backend's final assistant message, so a delegate that summarizes and points at earlier turns loses every citation.
- **Say whether the delegate may fan out.** Codex has self-dispatched nested consults unprompted, and nested trailers in one log confuse `agent-wait.sh`'s anchors. Either "do this yourself; do not dispatch other agents or run any agent CLI", or an explicit instruction to use internal subagents when breadth is wanted (one dispatch fanning out internally, never several parallel dispatches of the same mission). The injected consult preamble only suppresses test/build runs; it does not cover this.
- **Route external-fact questions** (does this CLI flag take a value?) to codex or cursor, whose consults can run `<cli> --help`. claude and copilot consults are sandboxed and correctly answer UNPROVEN — a capability gap, not a disagreement. Weight a verdict backed by captured help output over static reasoning.
- **Consults cannot write; `work` takes the worktree lock.** For parallel authoring, dispatch consults that return content in delimited blocks and write the files yourself — N-way parallelism with every repo write under one hand.
- **One fix agent per review round, confirm-then-fix.** Consolidate a panel's findings into one `work` dispatch that reports per finding `confirmed+fixed` or `refuted+skipped+<reason>`; fix prompts name the *capability* to close, not the reviewer's repro spellings (see `docs/agent_notes/backlog/code-quality-2026-08-01/DRAIN.md` step 6).
- **Guard/hook code reviews need a threat model**: "a tripwire against accidental drift by cooperative agents, not a security boundary — rate deliberate-evasion findings P1 only if plausibly unintentional", or the round never converges.

Concurrency: keep mutating `work` lanes to one or two at a time; a flat read-only panel of ~5 consults is fine. When the container OOMs, look for a heavy loop *inside* one delegate (a subprocess-spawning parity harness, an unfiltered test run) before blaming fan-out — both 2026-08 OOMs first attributed to panel width were traced to a single delegate.
...
`run_in_background=true`, end your turn, and let the background-completion
notification re-invoke you. Reach for `agent-wait.sh` only when you cannot
idle: a lost dispatch, someone else's run, dead-run triage, or a Workflow
shim.

**agent-cli inside Workflow scripts.** Full guidance in
[references/claude-workflows.md](references/claude-workflows.md). The short
version: when the orchestrator's main loop can own the wait, dispatch
agent-run.sh directly via background Bash — one backgrounded dispatch per
worktree gives parallelism without shims. Reserve thin `agent()` shims for
short dispatches the idle enforcer will not outlive (it kills a shim around
the hour mark, orphaning its delegate).
