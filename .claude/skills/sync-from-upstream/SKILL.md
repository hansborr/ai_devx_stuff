...
| `AGENTS.md`, `CLAUDE.md` | This clone's own agent orientation ("partial mirror; here's what works and what doesn't"), not upstream's product brief. |
| `docs/upstream-AGENTS.md` | Generated by the sync script itself on `--apply`: upstream's `AGENTS.md` plus a provenance header, kept as an example of the feedforward-context pattern. Excluded from the mirror passes so the delete pass never orphans it. |
| `bun.lock` | Locks this clone's actual manifests (root + stub `packages/**`), so `bun install` leaves a clean tree and frozen installs work. Upstream's lockfile pins ~963 lines of workspace deps for app code that isn't here. Refreshed by `bun install`, not by the mirror. |
...
> Note: package.json and the generated files
...
> you actually run the ratchet locally (regenerate it then). Similarly,
> `bun run harness:check` reports the local-only `sync-from-upstream` skill as
> uninventoried drift — also expected.
...
5. **Refresh the lockfile** if the sync changed any `package.json`:

   ```bash
   bun install
   ```

   `bun.lock` is preserved (it locks this clone's stub manifests, not
   upstream's full workspace), so `bun install` is how it picks up mirrored
   dependency changes. Include the resulting `bun.lock` diff in the sync
   commit.

6. **Hand off the commit.** This is a partial clone, so the Husky `commit-msg`
