#!/usr/bin/env bash
# Git merge-driver body for the demo's lint-ratchet baseline.
#
...
# embedded below. The clone-local merge config passes the driver key as the first
# argument, followed by Git's %O %A %B %L %P placeholders.
#
...
# falls back to the manual-recovery recipe and exits nonzero so Git still records
# the path as conflicted.
#
# The demo installs one baseline driver (lint-ratchet); a repo with more drivers
# would add cases here keyed by the metric baked into merge.<name>.driver.
set -euo pipefail
