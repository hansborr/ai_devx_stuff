...
export function formatRuleSourceDriftClassification(
  ...
  if (changedCount === 0 && infoCount === 0) {
    ...
    if (!equalCountSwapsProvable) {
      return (
        `lint:ratchet rule source identity drift only — finding counts unchanged from ${BASELINE_FILENAME} ` +
    ...
    return (
      `lint:ratchet rule source identity drift only — findings unchanged from ${BASELINE_FILENAME}; ` +
      "run bun run lint:ratchet:update to refresh ruleSourceHash metadata"
    );
  }
  if (changedCount === 0) {
