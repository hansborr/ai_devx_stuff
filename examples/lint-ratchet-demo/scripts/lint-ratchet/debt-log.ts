...
function isLegacyRemovalOnlyEntry(
...
function isAcceptedDebtRecord(entry: LintRatchetDebtLogEntry): boolean {
  return (
    (isAcceptedDebtLogEntry(entry) && !isLegacyRemovalOnlyEntry(entry)) ||
    isCoverageShrinkLogEntry(entry)
  );
}

function formatEntrySection(entry: LintRatchetDebtLogEntry, index: number): readonly string[] {
  if (isRetirementLogEntry(entry)) {
    const lines = [
  ...
  if (isMetricMigrationLogEntry(entry)) {
    return [
  ...
  if (isCoverageShrinkLogEntry(entry)) {
    return [
  ...
  if (!isAcceptedDebtLogEntry(entry)) return [];
...
export function formatLintRatchetDebtLogReport(
  ...
  const retirementCount = entries.filter(
    (entry) => isRetirementLogEntry(entry) || isLegacyRemovalOnlyEntry(entry),
  ).length;
  const lines = [
...
export function readLintRatchetDebtLog(contents: string): readonly LintRatchetDebtLogEntry[] {
  return parseDebtLogJsonl(contents, DEBT_LOG_FILENAME);
}
