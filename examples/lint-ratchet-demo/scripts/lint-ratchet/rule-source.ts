...
function readTypescriptEslintPackageVersion(): string {
  return readPackageVersion("typescript-eslint", "typescript-eslint package");
}
...
function readTypescriptPackageVersion(): string {
  return readPackageVersion("typescript", "TypeScript package");
}
...
function computeLocalLintRatchetRuleSourceHash(ratchet: LintRatchetConfig): string {
  ...
  try {
    ...
    const sourceIdentity = {
      ...
      eslintVersion: readEslintPackageVersion(),
      typescriptEslintVersion: readTypescriptEslintPackageVersion(),
      bareSpecifierVersions: closure.bareSpecifierRoots.map((packageRoot) => ({
        package: packageRoot,
        version: readPackageVersion(packageRoot, "local rule dependency package"),
      })),
    };
  ...
  switch (source.kind) {
    ...
    case "third-party": {
      ...
      const sourceIdentity = {
        ...
        eslintVersion: readEslintPackageVersion(),
        parserTypescriptEslintVersion: readTypescriptEslintPackageVersion(),
        ...
        ...(ratchetParserProfile(ratchet) === "type-aware-ts"
          ? { typescriptVersion: readTypescriptPackageVersion() }
          : {}),
      };
      const hash = createHash("sha256").update(JSON.stringify(sourceIdentity)).digest("hex");
      return `${LINT_RATCHET_CONFIG_HASH_PREFIX}${hash}`;
    ...
    case "core":
      return computeCoreLintRatchetRuleSourceHash(
        ratchet,
        readEslintPackageVersion(),
        readTypescriptEslintPackageVersion(),
      );
    default:
