...
import { complexityDelta, type LintRatchetComplexityFunction } from "./metrics.js";
...
// Improvement reason vocabulary produced here. The report formatter rebuilds its
// legacy `reason`-based improvement set (for envelopes without an explicit
// `kind`) from these exports plus `REMOVED_PATH_REASON`, so a reason added here
// can no longer silently misclassify in the report.
...
function lowerComplexityImprovement(
  ...
  return {
    ...
    baselineComplexity: context.complexityChange.baselineComplexity,
    currentComplexity: context.complexityChange.currentComplexity,
    reason: LOWER_COMPLEXITY_REASON,
  };
}
