...
import { relativePath, repoRoot } from "./paths.js";

const ESLINT_SEVERITY_ERROR = 2;
export const DEFAULT_COLLECT_CONCURRENCY = 3;
interface ItemsFromResultsOptions {
...
export function itemsFromResults(
  ...
  options: ItemsFromResultsOptions = {},
  ...
  for (const result of results) {
    const path =
      options.repoRootPath === undefined
        ? relativePath(result.filePath)
        : relativePathForRoot(result.filePath, repoRootPath);
...
export async function collectCurrentById(
  ...
  // The live registry lives here, at the outermost layer only; the scheduler
  // takes it as a parameter so it can be reused against any registry.
  ratchets: readonly LintRatchetConfig[] = lintRatchets,
): Promise<LintRatchetCurrentById> {
  const limit = Math.max(1, Math.min(concurrency, ratchets.length));
  const results = await collectRatchets(ratchets, ruleSourceHashesById, limit, trackedFiles);
