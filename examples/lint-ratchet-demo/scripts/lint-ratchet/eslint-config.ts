...
function renderLocalEslintConfig(ratchet: LintRatchetConfig): string {
...
function renderThirdPartyEslintConfig(ratchet: LintRatchetConfig): string {
...
function renderCoreEslintConfig(ratchet: LintRatchetConfig): string {
  const source = ratchetSource(ratchet);
  if (source.kind !== "core") {
...
function renderEslintConfig(ratchet: LintRatchetConfig): string {
  const source = ratchetSource(ratchet);
  switch (source.kind) {
    case "local":
      return renderLocalEslintConfig(ratchet);
    case "third-party":
      return renderThirdPartyEslintConfig(ratchet);
    case "core":
      return renderCoreEslintConfig(ratchet);
    default:
...
export function writeEslintConfig(
  ...
  const rendered = renderEslintConfig(ratchet);
