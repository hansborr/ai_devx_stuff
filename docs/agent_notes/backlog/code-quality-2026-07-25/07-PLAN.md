...
**Slice 07.1 is partially delivered from outside this plan**: SERVER-COMMENTS
slice S11 (merge `11af9d4c8`) created `packages/server/src/utils/string-order.ts`
and its test — the comparator is `compareCodeUnits`, per
[evidence correction 6](#evidence-corrections-claims-in-the-leaf-that-do-not-survive-checking)
— and repointed the four `spell-casting/` copies.
The 07.1 row below is annotated down to its genuine remainder; it is now a
comment/doc/test slice, not a creation one. 07.2-07.5 are untouched.

**Reconciliation status (2026-07-30):** items 1 and 2 of the
[index reconciliation](#index-reconciliation-slice-071-applies-these) are applied
— `00-index.md` no longer carries the `06 ↔ 07` edge or the "three streams"
description, and row 07 points here. Item 3 is applied in leaf 06's
`## Scope / caveats`. **No `Sev`/`Size` change is owed**: this plan re-scopes the
leaf into five slices without resizing it, so row 07's `XL` is settled rather
than pending, and 07.1 landing does not change it.
...
|---|---|---|---|
| **07.1** | **Partially delivered by `feat/cq-seed-pipeline-and-metadata`:** `packages/server/src/utils/string-order.ts` and `string-order.test.ts` exist, and the four copies in `combat-transaction.ts`, `load-participants.ts`, `resolve-character-spell.ts`, and `spell-casting.ts` have been repointed; do not recreate or repoint them. Remaining work: delete the false "Caster first" sentence now directly above `collectAffectedCharacterIds`, correct the `MODULE.md` ordering wording to UTF-16 code-unit, and pin that ordering contract in the facade test. **Do not touch `seed/spell-parser/extract-spell-combat.ts`.** | `grep -rn "function codePointCompare" packages/server/src/services/spell-casting/` returns nothing; a test pins the `affectedCharacterIds` ordering contract named in `spell-casting/MODULE.md`; the seed copy is untouched | `bun run test -- packages/server/src/services/spell-casting/spell-casting.test.ts packages/server/src/services/spell-casting/load-participants.test.ts` |
