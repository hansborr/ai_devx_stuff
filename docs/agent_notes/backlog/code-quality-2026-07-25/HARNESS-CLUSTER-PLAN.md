...
| H11 **[G]** | Open | **Create `scripts/worktree-db/` with one seam** (leaf 29 step 4, shrunk). Move the tombstones/allocations/state-codec cluster H1 just cleaned into `scripts/worktree-db/state.sh`, sourced unconditionally at top level from the `scripts/worktree-db.sh` facade. Give the part an `__MUSI_WTDB_STATE_SOURCED` guard and **forbid part-to-part sourcing** — the `readonly` block at `scripts/worktree-db.sh:88-138` aborts if a part is reachable by two sourcing paths. Do **not** move the merge-driver installers — one seam only, and nothing pins them any more: the text-matching check that used to read `scripts/worktree-db.sh` by hardcoded path is gone, so a slip there fails silently. Add the new part to the `# smoke-subjects:` headers of `scripts/tests/test-worktree-drop-gc.sh` and `scripts/tests/test-worktree-locking.sh` — the two suites that exercise the state codec after code-quality-2026-08-01 leaf 063 split the old single worktree smoke into four — and regenerate. **Stop after one seam**; further seams are opportunistic, not scheduled. | Facade sources the part; all four sourcing consumers still resolve every callee; installers untouched | `bash scripts/tests/test-worktree-db.sh`; `bash scripts/tests/test-worktree-new.sh`; `bash scripts/tests/test-worktree-drop-gc.sh`; `bash scripts/tests/test-worktree-locking.sh`; `bun run test:scripts:subjects`; `bun run test:scripts:subjects:check`; `bun run harness:check` |
...
2. **H11 is the most dangerous scheduled slice.** `worktree-db.sh` provisions
   ...
   `bun run dev` for everyone. Two specific traps: the `readonly` block at
   `:88-138` aborts if a part is reachable by two sourcing paths; and the
   ...
   split that drops any *other* callee. Keep the four worktree shell smokes
   (`bash scripts/tests/test-worktree-{db,new,drop-gc,locking}.sh`) green on
   every commit — between them they source all four consumers and are the only
   cheap defence. One suite is not enough: leaf 063 split the former single
   entrypoint, so `test-worktree-db.sh` alone now covers only the DB/init/dev
   band.
