...
Status: **S1 and S2 landed 2026-07-28 on branch `feat/cq-server-socket-types`,
merge `672cef373`, closing leaf 02. S4, S3, S5 and S6 landed 2026-07-29 on branch
`feat/cq-broadcast-registry-cleanup`, merge `08d9443ad`, closing leaf 04 and
delivering leaf 05 steps 1, 2 and 4a. S7 and S8 landed 2026-07-29 on branch
`feat/cq-server-authz-and-spell-rule`, merge `48ff021ed`, closing leaf 05 and
delivering leaf 03 steps 1, 2, 3, 5 and 7 — leaf 03 stays open on the optional
S17 and S18. S9, S10 and S11 landed 2026-07-29 on branch
`feat/cq-seed-pipeline-and-metadata`, merge `11af9d4c8`, discharging **every
slice this plan schedules for leaf 06** — steps 5-8 were parked by the
[leaf disposition](#leaf-disposition), not deferred, so leaf 06 is delivered as
far as it is schedulable. S12 and S13 landed 2026-07-30 on branch
`feat/cq-server-comments-s12-s13`, merge `3b7830ce4`, closing leaf 44.
S14-S16 are implemented on branch `feat/cq-server-comments-s14-s16` with the
merge `a01edb455`, closing the scheduled remainder; S17-S20 remain optional — the
optional S4 was taken.** See [Slices](#slices), whose
`State` column is the authority, plus [First landing outcome](#first-landing-outcome),
[Second landing outcome](#second-landing-outcome),
[Third landing outcome](#third-landing-outcome) and
[Fourth landing outcome](#fourth-landing-outcome). The
[index reconciliation](#index-reconciliation-applied-2026-07-28) this plan
carried was applied by the first landing.

Originally planned as: **the server's boundaries are in the right places and none of
...
break the moment they are given the package script leaf 06 asks for.**
**Both of those defects are since fixed** — leaf 51 (`4866df17b`) and leaf 52
(`455bac12e`), both in merge `6246c73cf`. Do not quote either from this paragraph
without reading
[Two live defects](#two-live-defects-the-leaves-do-not-record), which retracts
them. Supersedes
...
Both came out of the codex consult and both were verified against the tree as it
stood at planning time (`2cf49496`). **Neither is live any more, and the record
below is history, not a description of the tree.** Both were closed by merge
`6246c73cf`: defect 1 by leaf 51, which put the cap check and the write inside
one Serializable transaction, and defect 2 by leaf 52, which made all four seed
generators cwd-independent. Read each entry with the correction that follows it
before quoting it anywhere.

**1. The prepared-spell limit is a check-then-act race.** [P2]
**Fixed — do not repeat this claim.** See the correction below the paragraph.
...
**Correction (2026-07-29, applied by the S7 delivery).** The reframing above
survives; the disclaimer it prescribes does not. Leaf 51 landed in `6246c73cf`
and closed the race in full for this path: `routers/character-spell.ts` no longer
counts or updates at all — the lines this entry anchors do not exist — and
`utils/prepared-spell-toggle.ts` re-reads the row, computes the cap and counts
prepared spells inside one `isolationLevel: "Serializable"` transaction with a
six-attempt retry budget. No number of concurrent `togglePrepared` calls can now
raise a character's prepared count above its cap. **S7 must therefore not state
that the race survives, and must not file the serialization follow-on the row
asks for**; both would be false claims. The one thing S7's disclaimer was
protecting against still holds and is still worth saying: the extraction is
testability, not a concurrency change, and the guarantee belongs to
`prepared-spell-toggle.ts`. What remains genuinely unclosed is a *rules* gap, not
a race — character creation writes prepared level-1 spells with no cap check, so
a fresh wizard can start over its cap. That is [leaf 58](./58-creation-writes-over-prepared-cap.md),
already filed, and it is not S7's.

**2. All four seed generators derive their paths from `process.cwd()`, so leaf 06
step 2's package script would break them.** [P1]
**Fixed — do not repeat this claim.** See the correction below the paragraph.
`generate-class-features.ts:18`,
...
**Correction (2026-07-29, applied by the S7/S8 delivery).** The precondition is
**discharged**, not pending. Leaf 52 landed in `455bac12e` (merge `6246c73cf`)
and made all four generators cwd-independent: every path they read or write now
resolves through `seed/srd-generator-paths.ts`, whose `SEED_DIR` is
`resolve(import.meta.dirname)` and whose `REPO_ROOT` walks up from it, with
`seed/srd-generator-paths.test.ts` pinning the walk against committed markers.
`generate-class-features.ts`, `generate-subclasses.ts`, `generate-srd-spells.ts`
and `generate-srd-rules-glossary.ts` each import their constants from that module
and hold no `process.cwd()` — the lines this entry anchors do not exist. **S10
must therefore not schedule commit 1 as work**; leaf 06 step 2 can add the package
scripts directly. What the leaf's own caveat still owes is unrelated to cwd:
byte-identical regenerated output was never proven, because that needs the
optional `docs/refs/` checkout.
...
4. **Leaf 05 step 5's "~50 lines to `utils/`" is not all extractable as a pure
   ...
   the check-then-act race recorded above. **The race half is retracted** — leaf 51
   (`4866df17b`, merge `6246c73cf`) moved both reads and the write into one
   Serializable transaction in `utils/prepared-spell-toggle.ts` and the anchors
   above no longer exist. The half that decided S7's shape survives: the two
   Prisma reads are not part of the rule, so only `calcMaxPrepared` and
   `assertPreparedLimit` moved to `utils/prepared-spells.ts`.
5. **Leaf 06 step 2 as written breaks all four generators** — they resolve paths
   from `process.cwd()`. See the second live defect above. **Retracted** — leaf 52
   (`455bac12e`, merge `6246c73cf`) routed all four through
   `seed/srd-generator-paths.ts`, so step 2 is now safe as written.
...
- **`S1 → S2` (hard) — discharged.** S2's port references `AppSocketServer`
  through the augmentation S1 declares, and `getSocketIO`'s narrowing depends on
  `io` being a typed optional member. Both landed in `672cef373`, S1 first.
- **`S5 → S6` commit 3 (hard — this is the `04 → 05` edge) — discharged.** S5
  dissolves `services/rest-encounter-attribution.ts`, one of the three internal
  files leaf 05 counts against `services/README.md:21-28` criterion 3. Both landed
  in `08d9443ad`, S5 first, and `rest-MODULE.md` was written against the two files
  that remain. **The promotion itself (leaf 05 step 4b) was not scheduled and did
  not happen** — with two internal files criterion 3 fails and rest stays a flat
  service with a corrected companion doc.
- **`S2 ↔ S5` (hard, unrecorded in the index) — resolved by S2 landing first.**
  S2 retyped `rest-encounter-attribution.ts:37`'s `server: unknown` to the port,
  so S5 now deletes an already-typed parameter rather than an untyped one. No
  sequencing question remains.
- **`S4 → S3` commit 2 (soft) — discharged.** Both happened, and the registry
  split went first, as the edge asked: it changes what a thin per-family test can
  usefully assert.
- **`S8 → S17 → S18` (hard, if the optionals happen) — S8's side is discharged.**
  S17 and S18 both assume the union is gone and `AuthzCallerContext` is
  exported; both are true as of `48ff021ed`. `S17 → S18` still holds between the
  two optionals. This is now the only unmet edge among the optionals, and — with
  `S10 → S11` discharged by `11af9d4c8` — one of only two edges left in the
  cluster, the other being the soft `S12 → S13`.
- **`S10 commit 1 → S10 commit 3` (hard, inside the slice) — discharged.** cwd
  independence before the package scripts, or the scripts ship broken. Leaf 52
  landed the independence ahead of the slice (`455bac12e`, merge `6246c73cf`), so
  the edge has no unmet side: commit 3 can be written whenever S10 starts.
- **`S10 → S11` (soft) — discharged.** Both landed in `11af9d4c8`, S10 first, and
  the ordering earned its keep: S10's one-line generated-file change moved the
  baseline S11 had to hold byte-identical, which is only checkable because the
  attestation went in first.
- **`S11 ↔ 07-PLAN slice 07.1` (soft) — resolved by S11 owning the home.** 07.1
  had not landed, so S11 made the call `07-PLAN.md` left it: it created
  `packages/server/src/utils/string-order.ts` (comparator `compareCodeUnits`) and
  repointed the four `services/spell-casting/` copies, which is 07.1's file
  deliverable. 07.1's row is annotated to its remainder; it is now a
  comment/doc/test slice, not a creation one.
- **`S12 → S13` (soft, file-family).** Both touch harness prose; S12's guard is what
  makes S13's corrections stick. **This is the only edge left among the scheduled
  slices, and S12 is its head — nothing gates S12.**
...
- **Everything else is parallel.** S6, S7, S9, S12, S13, S15, S16 and S20 have no
  *incoming* edges. Of the scheduled slices still open, S12, S13, S14, S15 and
  S16 are all unblocked today.
...
## Index reconciliation (applied 2026-07-28)

**Done.** Written to be applied by whichever slice landed first; applied by the
S1/S2 landing (merge `672cef373`) as a bookkeeping pass. Kept here as the record
of what changed and why — **do not re-apply it.** Three items changed shape on
application, because the index was restructured after this list was written:

- **Item 2 was already discharged.** The "Read this first" section no longer
  exists; the index's `Plan` column points every one of these eight rows at this
  file and `## Cluster state` names the server/comments cluster beside the other
  three, so the set was already complete.
- **Item 4 was applied in the past tense.** S1 landed with S2 in the same merge,
  so instead of naming S1 as the next candidate the index's `## Next up` now
  names the *next* slice.
- **Item 5 was already true.** `AUDIT-SUMMARY.md` (which now owns the executive
  summary) already lists the prepared-spell race as a landed defect — leaf 51
  landed in `6246c73cf` — so only the "confirmed by two independent consults"
  clause on the layering claim was added.
...
| **Extract the prepared-spell predicates and call the limit fixed** | ~~S7 does the extraction and the commit message must say the opposite. The count at `character-spell.ts:207-213` and the update at `:225-228` remain a check-then-act race outside any transaction; a tidier `utils/` helper does not serialize anything. Serializing it is a separate decision that gives `togglePrepared` a transaction boundary and re-answers `services/README.md:253-265` Q1 in favour of a flat service.~~ **Retracted — the serialization this row prescribes as follow-up work is already done.** Leaf 51 (`4866df17b`, merge `6246c73cf`) landed before S7 and put the row re-read, the cap computation, the `characterSpell.count` and the `update` inside one `isolationLevel: "Serializable"` `$transaction` in `utils/prepared-spell-toggle.ts:132`, behind a six-attempt retry on serialization failure; `utils/prepared-spell-toggle.test.ts` pins the last-slot invariant under a barrier and `utils/serializable-isolation.test.ts` pins the abort shapes. The anchors above do not exist. **Do not re-file or re-implement this race.** What survives is only the narrow claim S7 acted on: the extraction is testability, not a concurrency change, and the guarantee belongs to `prepared-spell-toggle.ts`. The still-open prepared-spell gap is a *rules* one — [leaf 58](./58-creation-writes-over-prepared-cap.md), character creation writing over the cap with no concurrency involved. |
...
| **Add the four generator package scripts without fixing `process.cwd()` first** | ~~All four generators set `ROOT = process.cwd()` and join repo-relative paths, so a package-scoped script resolves `packages/server/packages/server/src/seed/…`. Broken on arrival. S10 commit 1 is the precondition.~~ **Moot — the precondition is discharged.** Leaf 52 (`455bac12e`, merge `6246c73cf`) routed all four generators through `seed/srd-generator-paths.ts`, which anchors on `import.meta.dirname`; no generator reads `process.cwd()`. Adding the package scripts is now the plain, safe form of S10 commit 3, and this row is history rather than a live constraint. |
