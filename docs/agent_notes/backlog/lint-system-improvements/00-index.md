...
These tasks have landed and remain listed in ordering for historical context:

- `02` — composite CI lint floor; CI keeps one `bun run lint` step instead of
  separate `lint:shell` / `lint:config-sensors` steps.
- `01` — CI coverage-map gate; validate now runs the full-tree
  `docs:lint-coverage-map:check` gate after lint-ratchet.
- `03` — ratchet CI pass deduplication; CI runs `lint:ratchet` and
  `lint:ratchet:zero-baseline`, with registry preflight folded into
  `lint:ratchet`.
- `04` — resolved by task 03; `verify:changed` matches the simplified CI
  ratchet surface.
- `05` — linted script reinclude patterns are derived from
  `lintedScriptFiles`, with flat-config directory unignores generated by the
  helper and `scripts/vitest.config.ts` kept as the named config-file extra.
- `08` — parallel runner ownership documented; `parallel-runner.sh` and
  `parallel-step.sh` remain separate because their contracts differ.
- `09` — agent hook formatter, linter, and commitlint invocations now use the
  local `node_modules/.bin` toolchain instead of external package resolution.
- `13` — warning severity semantics documented; normal ESLint warnings remain
  gate-failing under `--max-warnings=0`, while agent-envelope warnings are
  advisory.
- `15` — ESLint entrypoint policy re-export removed; drift tooling imports
  the E2E selector allowlist from `eslint-config/shared-policy.js` directly.
- `16` — lint-agent compatibility aliases retired; package scripts expose only
  the explicit local-rule envelope names while the diagnostics envelope tool id
  remains `lint:agent`.
- `18` — narrow Biome advisory decision; ESLint remains authoritative while
  Biome can be revisited only as an opt-in lint-only advisory tier.
...
1. `01-ci-coverage-map-gate.md` - **done** — full coverage-map gate in CI.
2. `02-ci-lint-step-deduplication.md` - **done** — composite floor.
3. `03-ratchet-ci-pass-deduplication.md` - **done** — drop check-baseline, fold
   check-registry.
4. `04-verify-ratchet-ci-parity.md` - **done** — parity achieved through task
   03.
5. `05-derive-linted-script-reinclude-patterns.md` - **done** — derive
   flat-config reinclude patterns from the linted script surface.
...
11. `08-parallel-runner-unification.md` - **done** — document separation, do
    not unify.
12. `09-agent-hook-pinned-tools.md` - **done** — use the pinned local
    formatter, linter, and commitlint binaries.
...
16. `13-warning-severity-semantics.md` - **done** — document normal ESLint
    warning enforcement versus agent-envelope warning behavior.
17. `14-max-lines-exception-lifecycle.md` - classify and report max-lines
    exceptions by lifecycle.
18. `15-eslint-entrypoint-exports.md` - **done** — drift tooling imports
    shared policy directly.
19. `16-lint-agent-alias-retirement.md` - **done** — legacy package aliases
    retired.
...
23. `20-lint-process-pain-points.md` - address the remaining lint-process DX
    pain points in recommended fix order.
