...
`bun run code:intel --` is implemented as a one-shot, read-only CLI with
`def`, `exports`, `dependents`, and `tests`. The review follow-ups for runtime
type-only edge filtering, bounded tests, project filters, and usage polish are
landed.
