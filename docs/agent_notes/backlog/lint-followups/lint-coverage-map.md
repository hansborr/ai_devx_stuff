...
| `scripts/drift-ai.test.ts` | 1 .ts | no (in `tsconfig.scripts.json`, but not in `eslint.config.js` re-includes) | `ratchet/local-type-assertion-boundary`, `ratchet/vitest-expect-expect-drift-ai-tests`, `ratchet/vitest-valid-expect-drift-ai-tests` | parser already wired via `tsconfig.scripts.json`; blocker is the ESLint re-include / ratchet entry | none - bug-class rules now floored | ratcheted | `finished_work/lint-hardening-leaf-41-drift-ai-test-bug-class-ratchets.md` |
