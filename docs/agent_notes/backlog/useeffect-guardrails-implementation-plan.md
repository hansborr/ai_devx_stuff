...
Related: `docs/agent_notes/backlog/lint-followups/15-react-deferred-rules.md`,
...
1. Add `eslint-plugin-react-hooks` → `react-hooks` to
   ...
   `scripts/lint-ratchet-config.ts`. Verify the plugin's export shape
   (`pluginExport: "default"` expected) against the installed `^7.1.1`.
2. Add the registry entry. Import and reuse the existing client source and
   test/helper glob constants from `eslint-config/shared-policy.js` so the
   ratchet matches the Leaf 15 inventory scope. Sketch (verify fields against
   the registry types and the constant names against `shared-policy.js`
   before landing):
...
3. `bun run lint:ratchet:update`, review the baseline diff (expect ~24
   findings matching the Leaf 15 inventory distribution), then
   `bun run lint:ratchet`.
4. Temporary-violation probe per the guide: add one synchronous
   `setState`-in-effect in an in-scope file, prove the ratchet reports it,
   revert.
5. Keep `"react-hooks/set-state-in-effect": "off"` in
   `eslint-config/client-configs.js` (the ratchet is the owner; avoid double
   reporting), and update the comment there to point at the ratchet id.
6. Update `15-react-deferred-rules.md` and append an `evaluation-verdicts.md`
   row: defer-for-error stands, ratchet-tier adopted, with the baseline count.
...
4. **Triage per rule** against the existing stop conditions
   (`docs/agent_notes/backlog/lint-hardening/14-react-hooks-broadened.md`):
   noise >5:1 over real findings → reject that rule; clean or near-clean
...
5. Append per-rule rows to `evaluation-verdicts.md` either way — adopted,
   deferred, or rejected — so the plugin is not blindly re-tried.
...
One short block, in `AGENTS.md` (Code Standards) or a
`docs/guides/client-effects.md` referenced from it:
...
Each slice follows the standard ratchet verification: registry preflight,
baseline update + diff review, `bun run lint:ratchet`, violation probe for
zero-or-frozen scopes, `bun run verify:changed`.

## Exit / revisit conditions

- If the React team promotes a future `no-deriving-state-in-effects` (probed
  as non-preset in Leaf 14) into the recommended presets, re-evaluate
  Recommendation 2's overlap before adopting more third-party rules.
