...
| 4 | Aggregate context-budget reporter (sum always-loaded session tokens) | Medium | M | Parked plan (rec M2) | Per-file caps exist; no command sums total always-on context — backs the "budget context like memory" claim. |
| 5 | Add scoped Stryker mutation + survivor summarizer to the weekly drift lane | Medium | M | Parked tasks (25 + backlog item 10) | The lane already runs; fold scoped mutation in and rank survivors for a recurring behavior signal. |
