...
**Repo example:** `docs/ai-harness.md` **Current Gaps** — the team self-declares behavior confidence its weakest harness axis, in the same document that maps all its guides and sensors.
...
**Repo example:** `docs/ai-harness.md` **Promotion Rule** + `scripts/verify/steps.generated.sh` (one 8-step matrix, four consumers, generated from one schema).
