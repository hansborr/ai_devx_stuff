...
- `/workspace/docs/generated/harness-controls.md:1476,1536,1576,...` — generated (exported) doc embeds `bash $CLAUDE_PROJECT_DIR/.claude/hooks/bun-run-quiet.sh`, `.codex/hooks/pre-tool-use.sh`, etc.; lines 1033/1271/1299 link `docs/agent_notes/lint-coverage-map.md` as "Paired guide". All targets are export-ignored.
...
1. **Un-ignore the harness-critical subset**: drop the `/.claude/**` and `/.codex/**` export-ignores (they are configuration, not process notes), keep `docs/agent_notes/**` ignored — but then either move durable, referenced docs (at minimum `docs/agent_notes/lint-coverage-map.md`, arguably the active backlog packs the harness docs cite) out to `docs/`, or carve narrow un-ignore exceptions for them (`.gitattributes` supports later-pattern overrides: `/docs/agent_notes/lint-coverage-map.md -export-ignore`).
