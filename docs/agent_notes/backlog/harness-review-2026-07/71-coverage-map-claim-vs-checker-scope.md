...
- The map already has *unverified* rows for these: `/workspace/docs/generated/lint-coverage-map.md:355` (`.husky/*`, "linted" via ShellCheck), `:376` (`.env.example`, excluded), `:417` (`index.html`, excluded), `:418` (`app.css`, excluded), `:419` (`bun.lock`, not-code), `:313` (`.jsonl` fixtures), `:411` (`.pdf`).
