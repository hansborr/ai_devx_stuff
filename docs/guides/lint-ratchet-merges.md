...
driver. The subsections below separate the package-owned ratchet merge handling
from Musi's three other generated baselines (knip unused-exports,
near-duplicate pairs, and max-lines exceptions). They share the same operating
model while the package owns the ratchet implementation and Musi retains the
other three.

## Ratchet baseline: portable merge handling

This is the package-owned part: the merge behavior and conflict recipe for
...
package executable described further below.
...
generated from the package's `git-rail/conflict-recovery.js` renderer by
`bun run docs:baseline-conflict-recipes`, so the runbook and packaged driver
never drift:
...
blocks are generated by `bun run docs:baseline-conflict-recipes`, keyed by
baseline. The lint-ratchet block comes from the package renderer above; the
three Musi-owned blocks come from `scripts/git/baseline-merge-driver.sh`'s
`print_conflict_recovery` cases. Each runbook block therefore follows its live
...
steps below apply to all four generated baselines alike. The package owns these
operations for lint-ratchet, including `baseline:restore-stage`; Musi's root
registry and dispatchers retain the corresponding wiring for its three other
baseline families.
...
The lint-ratchet, knip unused-exports, and near-duplicates tools expose
dedicated exit codes for their stale, summary-drift, and corrupt-baseline
verdicts. Near-duplicates reserves exit 6 for unreviewed working-tree baseline
growth over `HEAD`; pre-push blocks both that verdict and
whole-tree staleness. Truth-up and pre-push classify those codes only; stdout
and stderr are human-facing presentation and are never parsed for policy.
Truth-up treats codes 1 and 2 as unclassified failures, so an uncaught Bun
failure cannot masquerade as a stale baseline. If a future hook needs structured payload fields rather than a
verdict, extend the shared HarnessDiagnostics envelope with verdict-as-data
instead of inventing a per-tool status file.
