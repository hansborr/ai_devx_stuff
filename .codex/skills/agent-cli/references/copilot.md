...
- The wrapper parses Copilot's JSONL events and writes only the root agent's final assistant response to an attempt-private candidate; `python3` is required. After successful parsing it publishes the complete candidate atomically to the public answer path.
