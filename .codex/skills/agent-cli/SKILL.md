...
These caveats apply when the caller is **Codex**; a caller on another harness
ignores them and reads its own tree's section instead.
...
**Locks and sessions.** Every codex run — consults included — holds the
worktree lock: codex has no working OS sandbox in this devcontainer, so its
read-only-ness is prompt discipline verified by the drift check, not
prevention. Parallel codex dispatches therefore serialize on the lock; give
each its own worktree (set `workdir` per `exec_command`). To resume after a
lost session, use the `agent-run: session-id:` trailer when the run finalized,
otherwise codex's native session store at `~/.codex/sessions/`.
