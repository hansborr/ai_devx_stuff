...
import { renderBaselineMergeAttributes, UnterminatedBlockError } from "./info-attributes.js";
...
describe("renderBaselineMergeAttributes", () => {
  ...
  it("throws UnterminatedBlockError on an unterminated managed block instead of dropping trailing rules", () => {
    const current = [
      "unrelated/path merge=union",
      MANAGED_BEGIN,
      "/lint-ratchet.baseline.json merge=lint-ratchet-baseline",
    ].join("\n");
    expect(() => render(current)).toThrow(UnterminatedBlockError);
  });
