...
function isSortedUnique(values: readonly string[]): boolean {
  ...
  for (const value of values) {
    // Codepoint comparison, not localeCompare: these registry lists feed
    // configHash inputs, so the sorted/unique verdict must be identical on every
    // machine regardless of the runtime's ICU build or default locale. JS
    // relational operators on strings compare by UTF-16 code unit.
...
export function hasNormalizedPath(value: string): boolean {
  return (
...
function validateSortedPathList(
  ...
  if (!isSortedUnique(values)) {
    failures.push(`${ratchetId}: ${fieldName} must be sorted (by codepoint) and duplicate-free`);
  }
  for (const value of values) {
    if (value.startsWith("!")) {
      failures.push(`${ratchetId}: ${itemLabel} must not use ! negation: ${value}`);
    } else if (!hasNormalizedPath(value)) {
...
function validateRatchetEntry(
  ...
  const parserProfile = ratchetParserProfile(ratchet);
  ...
  validateRatchetRuleOptions(ratchet, failures);
...
// Shared metric/ruleId pairing rules so `--propose` rejects the same bad
// combinations at preview time that registry validation rejects at promotion —
// one source of truth, not two copies. Needs only id/ruleId/metric/source kind.
export function metricPairingFailures(
  id: string,
  ruleId: string,
  metric: LintRatchetMetric,
  sourceKind: LintRatchetRuleSource["kind"],
): string[] {
  const failures: string[] = [];
  if (metric === "effective-line-count" && ruleId !== "local/max-lines") {
    failures.push(`${id}: effective-line-count metric requires ruleId local/max-lines`);
  }
  if (metric === "complexity-severity" && (sourceKind !== "core" || ruleId !== "complexity")) {
    failures.push(`${id}: complexity-severity metric requires core ruleId complexity`);
  }
  return failures;
}
...
function validateRatchetModeAndMetric(
  ...
  // No mode-implemented check: LintRatchetMode is exactly the implemented set
  // now that the never-implemented `ratchet-down` is off the type surface.
  if (!IMPLEMENTED_METRICS.has(ratchet.metric)) {
    failures.push(`${ratchet.id}: metric ${ratchet.metric} is not implemented`);
  }
  failures.push(...metricPairingFailures(ratchet.id, ratchet.ruleId, ratchet.metric, source.kind));
}

function validateRatchetRuleOptions(ratchet: LintRatchetConfig, failures: string[]): void {
