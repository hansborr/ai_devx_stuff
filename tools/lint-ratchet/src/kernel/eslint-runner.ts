...
/**
 * Acknowledge a close() failure during best-effort fd cleanup. Closing an
 * already-invalid fd throws (e.g. the spawned child closed its inherited copy
 * first); such failures release nothing and run inside the child error/close
 * handlers, where there is no caller to surface them to and re-throwing would
 * escape an event handler. They are therefore intentionally not propagated —
 * genuine spawn/parse failures surface through their own reject paths instead.
 * A named acknowledgement keeps the caught error observable to a reader rather
 * than silently discarded in an empty catch.
 */
function acknowledgeBestEffortCloseError(_error: unknown): void {}
...
async function spawnEslint(
  ...
  const closeParentFd = (): void => {
    ...
    try {
      ...
      acknowledgeBestEffortCloseError(error);
    }
  };
...
export async function runEslintForFiles(
  ...
  for (const chunk of fileChunks(files)) {
    ...
    results.push(...(await spawnEslint(ratchet, args, binding.repoRoot)));
  }
  return results;
}
