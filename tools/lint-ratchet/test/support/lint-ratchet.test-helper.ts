...
// Shared lint-ratchet test fixtures for the package's own suites. The
// baseline/current shape (LintRatchetCurrentById, the rule-source-hash prefix)
// is exactly the contract that changes when a ratchet metric is added, so the
// fixture rule-source-hash const and the entries -> LintRatchetCurrentById
